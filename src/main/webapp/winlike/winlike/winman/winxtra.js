...
function submitWinLIKE_(a,b,c,d,e,f){
	...
	if(!c||c=='undefined'){
		c=document.getElementById(a);d=a_
	}
	if(c.tagName!='FORM'){
		e=c;
	...
	switch(fc_[ev_]){
		case 1:fc_[ev_]=0;
			return;
		case 2:b.l_?fc_[ev_]=3:fc_[ev_]=4;
			return F_;
		case 3:fc_[ev_]=4;
			return;
		case 4:fc_[ev_]=0;
	...
	if(eI_){
		...
		if(eH_) {
			...
			if(eJ_.es_){
				f?fc_[ev_]=1:document.getElementById(ev_).submit();b.eE_(el_,nr,eI_)
			}else{
				eJ_.b3_=nr;
				if(f){
					ew_=g1_();
