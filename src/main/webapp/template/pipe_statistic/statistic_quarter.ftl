<#include "statistic_quarter_head.ftl">
				 	...
				 	<tr class="table02">
				  	...
				  	<#list seq as x>
					  	...
					  	<tr class="table03" bgcolor=${color }>
			  		 	...
			  		 	<tr class="table03" bgcolor=${color }>
			  		 		<td><strong>未季报单位数</strong></td>
				  		 	<#if areas?exists>
							  	<#list areas?if_exists as a><td height="22" onclick="showUnCompanyListDetail(${a.areaCode}, '<#if x!=100>${x}</#if>');" style="cursor: pointer;" onMouseOver="this.style.backgroundColor=trMoveColor;" onMouseOut="this.style.backgroundColor='';" title="查看数据详情">{$T.returnMap${x}.s_${x}_${a.areaCode}_q - $T.returnMap${x}.s_${x}_${a.areaCode}_y}</td></#list>
				  		 		<td onclick="showUnCompanyListDetail(0, '<#if x!=100>${x}</#if>');" style="cursor: pointer;" onMouseOver="this.style.backgroundColor=trMoveColor;" onMouseOut="this.style.backgroundColor='';" title="查看数据详情">{<#list areas?if_exists as a>($T.returnMap${x}.s_${x}_${a.areaCode}_q - $T.returnMap${x}.s_${x}_${a.areaCode}_y) + </#list> + 0}</td>
				  		 	</#if>
					  	</tr>
					  	<tr class="table03" bgcolor=${color }>
			  		 		<td nowrap><strong>季&nbsp;报&nbsp;率（%）</strong></td>
				  		 	<#if areas?exists>
							  	<#list areas?if_exists as a><td height="22">{#if $T.returnMap${x}.s_${x}_${a.areaCode}_q != 0} {(100 * $T.returnMap${x}.s_${x}_${a.areaCode}_y / $T.returnMap${x}.s_${x}_${a.areaCode}_q).Fixed(2)} {#else} 0/0 {#/if}</td></#list>
							  	<td>{#if (<#list areas?if_exists as a>$T.returnMap${x}.s_${x}_${a.areaCode}_q + </#list> + 0) != 0} {(100*(<#list areas?if_exists as a>$T.returnMap${x}.s_${x}_${a.areaCode}_y + </#list> + 0) / (<#list areas?if_exists as a>$T.returnMap${x}.s_${x}_${a.areaCode}_q + </#list> + 0)).Fixed(2)} {#else} 0/0 {#/if}</td>
				  		 	</#if>
					  	</tr>
					</#list>
				</table>
...
<script>
	...
	function showAreaDetail(areaCode) {
		window.location = "loadQuarter.xhtml?statistic.remark=${statistic.remark}&statistic.year=${statistic.year}&statistic.quarter=${statistic.quarter}&statistic.areaCode=" + areaCode;
	}
	
	function changeUrl(quarter) {
		window.location = "loadQuarter.xhtml?statistic.areaCode=${statistic.areaCode}&statistic.remark=${statistic.remark}&statistic.year=${statistic.year}&statistic.quarter=" + quarter;
	}
	
	function changeUrl2(remark) {
		window.location = "loadQuarter.xhtml?statistic.areaCode=${statistic.areaCode}&statistic.year=${statistic.year}&statistic.quarter=${statistic.quarter}&statistic.remark=" + remark;
	}

	function showMonthData(month) {
		window.location = "loadQuarter.xhtml?statistic.areaCode=${statistic.areaCode}&statistic.remark=${statistic.remark}&statistic.year=${statistic.year}&statistic.quarter=${statistic.quarter}&statistic.month=" + month;
	}	
	
	function showYearData(year) {
		window.location = "loadQuarter.xhtml?statistic.areaCode=${statistic.areaCode}&statistic.remark=${statistic.remark}&statistic.quarter=${statistic.quarter}&statistic.year=" + year;
	}
	...
	function showPipelineListDetail(areaCode, type) {
		<#if statistic?? && statistic.areaType?? && statistic.areaType=='third_area'>
			window.location = "statistic_pipelineList.xhtml?statistic.secondArea=${statistic.areaCode}&statistic.areaCode="+areaCode+"&statistic.lineType=" + type;
		<#else>
			window.location = "statistic_pipelineList.xhtml?statistic.areaCode="+areaCode+"&statistic.lineType=" + type;
		</#if>
	}	
	
	function showUnCompanyListDetail(areaCode, type) {
		<#if statistic?? && statistic.areaType?? && statistic.areaType=='third_area'>
			window.location = "statistic_unCompanyList.xhtml?statistic.year=${statistic.year}&statistic.quarter=${statistic.quarter}&statistic.month=${statistic.month}&statistic.secondArea=${statistic.areaCode}&statistic.areaCode="+areaCode+"&statistic.lineType=" + type;
		<#else>
			window.location = "statistic_unCompanyList.xhtml?statistic.year=${statistic.year}&statistic.quarter=${statistic.quarter}&statistic.month=${statistic.month}&statistic.areaCode="+areaCode+"&statistic.lineType=" + type;
		</#if>
	}	
</script>
