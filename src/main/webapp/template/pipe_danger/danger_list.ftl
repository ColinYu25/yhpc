...
<div class="menu">
  	<ul>
		<li id="img_amend"><a href="javascript:window.location='createDangerInit.xhtml?entity.id=${entity.id}'" class="b2"><b>添加</b></a></li>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_list">
  ...
  <tr>
    ...
    <td nowrap><a href="loadDanger.xhtml?danger.id=${c.id}">${c.dangerNo}</a>&nbsp;</td>
    <td nowrap><div align="center">${c.dangerAdd}&nbsp;</div></td>
    <td nowrap>${c.description}&nbsp;</td>
    <td nowrap><#if (c.daPipeRollcallCompanies?size>0)>是<#else>否</#if></td>
    <#if !danger?if_exists.isGorver?exists>
    <td nowrap><#if (c.daPipeDangerGorvers?size>0)>是<#else>否</#if></td>
     </#if>
