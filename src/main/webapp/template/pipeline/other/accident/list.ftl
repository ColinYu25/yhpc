...
<div id="main">
	<div id="contents">
		...
		<div class="menu">
			...
			<li id="img_amend"><a class="b2" href="javaScript:loadNote('accident_update.xhtml?entity.id');"><b>修改</b></a></li>
...
<script>
	...
	function create(){
		...
		if (pipeLineId != ""){
			window.location.href = "accident_create.xhtml?entity.daPipelineInfo.id=" + pipeLineId;	
		}
	}
	...
	function del(){
		...
		for(var i=0; i<ids.length; i++){
			if (ids[i].checked){
				window.location.href = "accident_delete.xhtml?entity.type=3&entity.id=" + ids[i].value + "&entity.daPipelineInfo.id=" + pipeLineId;
				return;
			}
		}
		alert("请选择要删除的数据");
