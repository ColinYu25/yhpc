...
<div id="header">
	<ul id="primary">
		...
		<li><span style="width:60px;">政府协调</span></li>						
	</ul>
</div>
<div id="main">
	<div id="contents">
		...
		<div class="menu">
			...
			<li id="img_amend"><a class="b2" href="javaScript:loadNote('zfxt_update.xhtml?entity.id');"><b>修改</b></a></li>
		...
		<form action="zfxt_list.xhtml" method="post" name="companiesForm" id="companiesForm">
...
<script>
	...
	function create(){
		...
		if (pipeLineId != ""){
			window.location.href = "zfxt_create.xhtml?entity.daPipelineInfo.id=" + pipeLineId;	
		}
	}
	...
	function del(){
		...
		for(var i=0; i<ids.length; i++){
			if (ids[i].checked){
				window.location.href = "zfxt_delete.xhtml?entity.type=3&entity.id=" + ids[i].value + "&entity.daPipelineInfo.id=" + pipeLineId;
				return;
			}
		}
		alert("请选择要删除的数据");
