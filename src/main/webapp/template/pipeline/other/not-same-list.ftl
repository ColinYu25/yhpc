...
<div id="main">
	<div id="contents">
		...
		<div class="menu">
			...
			<li id="img_amend"><a href="javaScript:;" class="b2" onClick="loadNote('notSame_update.xhtml?entity.id');"><b>修改</b></a></li>
			<li id="img_del"><a href="javaScript:;" class="b3" onClick="del();"><b>删除</b></a></li>  	
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list">
			  ...
			  <#if result?exists>
				  ...
				  <tr>
				    ...
				    <td>
				    <#if item.attechFile?? && item.attechFile?length gt 0>
					    <a href="${base}${item.attechFile}">下载</a>	    
				    </#if>
				    &nbsp;
				    </td>
...
<script>
	...
	function create(){
		...
		if (pipeLineId != ""){
			window.location.href = "notSame_create.xhtml?entity.daPipelineInfo.id=" + pipeLineId;	
		}
	}
	...
	function del(){
		...
		for(var i=0; i<ids.length; i++){
			if (ids[i].checked){
				window.location.href = "notSame_delete.xhtml?entity.type=3&entity.id=" + ids[i].value + "&entity.daPipelineInfo.id=" + pipeLineId;
				return;
			}
		}
		alert("请选择要删除的数据");
