...
function sear_Form() {
	if(get("type").value=="3") {
		...
		window.location="loadProvinceReports.xhtml?entity.reportDateBegin="+get("reportDateBegin").value+"&entity.reportDateEnd="+get("reportDateEnd").value+"&&entity.addStatistic="+get("addStatistic").disabled;
	
	} else {
		window.location="../mine/loadProvinceReports.xhtml?threeReport.year="+get('year').value+"&threeReport.month="+get('month').value+"&entity.type="+get('type').value+"&&entity.addStatistic="+get("addStatistic").disabled;
	}
}
...
function addStatistics() {
	...
	var roleDepic = '${roleDepic}';
	if(roleDepic && roleDepic=="anwei"){
		window.location = '../dfzw/loadProvinceReportForCity.xhtml?entity.province=true&entity.anweiCheck=true&entity.reportDateBegin='+get('reportDateBegin').value+'&entity.reportDateEnd='+get('reportDateEnd').value+'&entity.reportType='+get('reportType_hidden').value;
	}else{
		window.location = '../dfzw/loadProvinceReportForCity.xhtml?entity.reportDateBegin='+get('reportDateBegin').value+'&entity.reportDateEnd='+get('reportDateEnd').value+'&entity.reportType='+get('reportType_hidden').value;
	}
}

function doneCityReport(id) {
	var roleDepic = '${roleDepic}';
	if(roleDepic && roleDepic=="anwei"){
		if(window.confirm('确定上报并而同时发送数据到省隐患排查吗？'))
			window.location='doneCityReport.xhtml?entity.report=true&entity.id=' + id ;
	}else{
...
<table width="98%" border="0" cellpadding="0" cellspacing="0" class="table_input">
  <tr>
	...
	至
	<input type="text" id="reportDateEnd"
			...
			readOnly="true" onchange="changeReportDateEnd(this.value);" onchange="changeReportDateEnd(this.value);"/>
	</td>
	...
	<input id=addStatistic  name=addStatistic  type="button" value="新增汇总统计" onClick="addStatistics()"  <c:if test="${hasAjjLookRole=='1'}">disabled="disabled"</c:if>/></td>
  </tr>
</table>
</form>
<table>
	 ...
	 <td>
	 	<c:choose>
	 		<c:when test="${viewRole}">
	 			<a href="#" onClick="window.location='view.xhtml?entity.id=${item.id}&entity.province=true';">查看</a>
	 		</c:when>
	 		<c:otherwise>
	 			<c:choose>
				 ...
				 <c:if test="${item.state==0}"><a href="#" onClick="if(window.confirm('  确定删除吗？'))window.location='delete.xhtml?entity.id=${item.id}&entity.province=true&entity.report=true';">&nbsp;删除</a>&nbsp;</c:if>
			 
				 <c:if test="${hasAjjLookRole=='0'}">
			    	<c:choose><c:when test="${item.isReport}"></c:when><c:otherwise><a href="#" onClick="doneCityReport(${item.id});"> <c:if test="${item.state==0}">上报</c:if></a>&nbsp;</c:otherwise></c:choose>
			 	</c:if>
			 	<c:choose><c:when test="${item.allowRollback}"><a href="#" onClick="if(window.confirm('  确定退回吗？'))window.location='rollback.xhtml?entity.id=${item.id}&entity.report=true';">退回</a></c:when><c:otherwise></c:otherwise></c:choose>
	 		</c:otherwise>
	 	</c:choose>
	 </td>
...
<script type="text/javascript">
if (${entity.addStatistic}==true){
	document.getElementById("addStatistic").disabled=true; 
}else{
	document.getElementById("addStatistic").disabled=false;	
