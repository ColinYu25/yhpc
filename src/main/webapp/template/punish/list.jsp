...
<body>
<form method="post" name="searchForm" id="searchForm"	action="../punish/list.xhtml">
...
<table width="98%" border="0" cellpadding="0" cellspacing="0"
	class="table_input">
	<tr>
		<td align="center" style="font-size: 18px" <c:if test="${entity.province}">colspan="3"</c:if><c:if test="${!entity.province}">colspan="5"</c:if>><strong>打非治违处罚报列表 </strong></td>
	</tr>
	<tr>
		<th width="15%">日期</th>
				...
				<c:otherwise>
					<c:choose>
						...
						<c:otherwise>
							<a href="#" onClick=" 
							     <c:choose>
								 	<c:when test='${item.state==2}'>window.location='../punish/view.xhtml?entity.id=${item.id}';</c:when>
					...
					<!--删除判断 -->
					<c:if test="${item.state=='0'}">&nbsp;<a href="#" onClick="if(window.confirm('  确定删除吗？'))window.location='delete.xhtml?entity.id=${item.id}';">删除</a></c:if> &nbsp;
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="#"
								onClick="if(window.confirm('  确定上报吗？'))window.location='doneReport.xhtml?entity.id=${item.id}';"><c:if test="${item.state=='0'}">上报</c:if></a>
						</c:otherwise>
					</c:choose> 
					<!--退回判断 -->
					...
					<c:choose>
						<c:when test="${anweiUserFlag=='1' && item.anweiBackState=='1'}">
								       <!--当前用户为安委会用户时，退回需要特殊处理-->
									    <a href="#" onClick="if(window.confirm('  确定退回吗？'))window.location='../punish/rollback.xhtml?entity.id=${item.id}&entity.report=true&anweiBackFlag=1';">退回</a>
						</c:when>
						<c:otherwise>
								<c:choose>
									<c:when test="${ !item.notAllowedroolBack && item.allowRollback && roleDepic == item.roleDepic }">
									    <a href="#" onClick="window.location='load.xhtml?entity.id=${item.id}';">修改</a>
										<a href="#" onClick="if(window.confirm('  确定退回吗？'))window.location='../punish/rollback.xhtml?entity.id=${item.id}&entity.report=true';">退回</a>
									</c:when>
