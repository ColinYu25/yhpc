...
<form name="mineForm" id="mineForm" method="post">
  <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" id="tb">
    <tr>
	  ...
	  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
		     <th colspan="2" height="50" class="head">煤矿、金属非金属矿山等工况企业安全生产事故隐患排查治理情况月报表</th>
        </tr>
	...
	<c:forEach var="item" items="${mineType}" varStatus="status">
		<tr id="tr_${item.key}">
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
     ...
     <input type="button" class="but_2" onClick="window.location='loadMine2012.xhtml?mineReport.id=${mineReport.id}&mineReport.print=true';" value="打  印" />
...
for(var i = 1; i < 16; i++){
	if(i!=3&&i!=6&&i!=9){//排除比率列
		...
		for (var z = 1; z < rows.length; z++){
			if (rows[z].id.indexOf("8")==-1&&rows[z].id.indexOf("104")==-1) {//排除危化品子项
...
for (var i=7; i<tr_heji.cells.length; i++) {
	if (i!=9 && i<15) {//排除百分率、资金
		if (tr_heji.cells[i].innerHTML!="&nbsp;") {
			tr_heji.cells[i].innerHTML = "<a href='#' onClick='openWindow(108,"+index+");'>"+tr_heji.cells[i].innerHTML+"</a>";
		}
		index ++;
	}
}
