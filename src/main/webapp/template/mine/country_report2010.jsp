...
<body>
<form method="post" name="searchForm" id="searchForm" action="loadCountry2010.xhtml">
...
<table width="98%" border="0" cellpadding="0" cellspacing="0" class="table_list">
  ...
  <c:forEach var="mine" items="${mineReports}" varStatus="status">
	<tr>
	 ...
	 <td>${mine.fillMan}&nbsp;</td>
	 <td>${mine.tel}&nbsp;</td>
	 <td><a href="javaScript:;" onClick="window.location='loadCountry2010.xhtml?mineReport.id=${mine.id}&mineReport.reportMonth=${mine.reportMonth}&mineReport.type=${mineReport.type}';"><c:choose><c:when test="${mine.state>0}">查看</c:when><c:otherwise>修改</c:otherwise></c:choose></a>&nbsp;<c:if test="${mine.state==0}"><a href="javaScript:;" onClick="if(window.confirm('  确定删除吗？'))window.location='deleteMine.xhtml?mineReport.id=${mine.id}&mineReport.report=true';">删除</a>&nbsp;</c:if><c:choose><c:when test="${!mine.allowReport}"></c:when><c:otherwise><a href="javaScript:;" onClick="if(window.confirm('  确定上报吗？'))window.location='doneCityReport.xhtml?mineReport.id=${mine.id}&mineReport.report=true';">上报</a>&nbsp;</c:otherwise></c:choose>
	 ...
	 <td><a href="javaScript:;" onClick="window.location='loadCountry2010.xhtml?mineReport.id=${mine.id}&mineReport.reportMonth=${mine.reportMonth}&mineReport.type=${mine.type}&mineReport.print=true';">打印</a></td>
	</tr>
  </c:forEach>
  <tr>
