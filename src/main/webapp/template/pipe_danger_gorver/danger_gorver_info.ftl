...
<table cellspacing="0" cellpadding="0" class="table_input">
  <tr>
    <th colspan="2">管道名称</th>
    <td colSpan="2">${danger.pipeLine.name}&nbsp;</td>
    <th>编　号</th>
    <td colspan="2">${danger.dangerNo}&nbsp;</td>
  </tr>
  <tr>
    <th width="7%" rowSpan="3" align="right"><strong>单　位&nbsp;</strong></th>
    <th width="7%" align="right"><strong>名　称&nbsp;</strong></th>
    <td colSpan="5">${danger.pipeLine.daPipelineCompanyinfo.company.companyName}&nbsp;</td>
  </tr>
  <tr>
    <th align="right"><strong>地　址&nbsp;</strong></th>
    <td colSpan="5">${danger.pipeLine.daPipelineCompanyinfo.company.regAddress}&nbsp;</td>
  </tr>
  <tr>
    <th align="right"><strong>区　域&nbsp;</strong></th>
    <td colSpan="5" id="companyArea">&nbsp;</td>
  </tr>
  <tr>
    <th rowSpan="2" align="right"><strong>隐　患&nbsp;</strong></th>
    <th align="right"><strong>地　址&nbsp;</strong></th>
  ...
  <tr>
    ...
    <td colSpan="2"><input maxLength="10" size="12" name="dangerGorver.money" id="money" value="${dangerGorver.money.toString()}" style="ime-mode:disabled">
      万元 <span class="red_point">*</span><ui:v for="money" rule="double" empty="&nbsp;" pass="&nbsp;" tips="&nbsp;" warn="&nbsp;"/></td>
  </tr>
  <tr>
    <th colspan="2">备　　　注</th>
...
<input type="hidden" name="dangerGorver.daPipeDanger.id" value="${danger.id}" id="dangerId"/>
</form>
<script>
var areaObj = new Area("${AreaXmlUrl}");
  ...
  <#if danger?exists>
 	<#if danger.pipeLine.daPipelineCompanyinfo.company.firstArea != 0>
		companyFirstArea=areaObj.getArea("${danger.pipeLine.daPipelineCompanyinfo.company.firstArea}")[0];
	</#if>
	<#if danger.pipeLine.daPipelineCompanyinfo.company.secondArea != 0>
		companySecondArea=areaObj.getArea("${danger.pipeLine.daPipelineCompanyinfo.company.secondArea}")[0];
	</#if>
	<#if danger.pipeLine.daPipelineCompanyinfo.company.thirdArea != 0>
		companyThirdArea=areaObj.getArea("${danger.pipeLine.daPipelineCompanyinfo.company.thirdArea}")[0];
	</#if>
  </#if>
