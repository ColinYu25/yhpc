...
<form action="loadNotice.xhtml?sysComit=${sysComit?string}" method="post" name="tradeTypeForm" id="tradeTypeForm">
...
<div class="menu">
	<div style="float:left;">
  	<ul>
	<li id="img_left"></li>
	<#if sysComit><li id="img_save">		<a href="#" class="b1" onClick="createTrade('createNoticeInit.xhtml?sysComit=${sysComit?string}');"><b>添加</b></a></li>
	<li id="img_amend">		<a href="#" class="b2" onClick="loadNote('createNoticeInit.xhtml?sysComit=${sysComit?string}&daActualTableNotice.id');"><b>修改</b></a></li>
	<li id="img_del">		<a href="#" class="b3" onClick="loadNote('deleteNotice.xhtml?sysComit=${sysComit?string}&daActualTableNotice.id');"><b>删除</b></a></li></#if>
  	...
  	<#list actualizeTableNoticeList as actualizeTableNotice>
  		<tr>
	    	...
	    	<td><#if pagination.itemCount?exists>${pagination.itemCount+actualizeTableNotice_index+1}<#else>${actualizeTableNotice_index+1}</#if></td>
  			<td><div align="left"><a href="createNoticeInit.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&sysComit=${sysComit?string}">${actualizeTableNotice.title}&nbsp;</a></div></td>
  			<td>&nbsp;${actualizeTableNotice.createTimeBegin}</td>
 			<!--<#if sysComit><td><#if actualizeTableNotice.auditing><a href="updateNotice.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&state=false&sysComit=${sysComit?string}">取消审核</a><#else><a href="updateNotice.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&state=true&sysComit=${sysComit?string}">审核</a></#if></td></#if>-->
  		</tr>
  	</#list>
  </#if>
</table>
