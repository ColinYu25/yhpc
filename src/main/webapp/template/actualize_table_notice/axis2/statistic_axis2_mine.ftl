...
<table width="30%" border="0" height="30" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<#if userDetail.secondArea==0>
		<td align="center">
			<input name="print" id="sendData" type="button" <#if send?exists>disabled="disabled"<#else><#if userDetail.secondArea!=0>disabled="disabled"</#if></#if> value="<#if send?exists>已经<#else>确定</#if>发送"  onclick="sendData();"/>
