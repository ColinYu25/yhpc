...
<table width="98%" border="0" cellpadding="0" cellspacing="0" class="table_4_blue">
  <tr>
	<th width="98%" height="22">表格下载列表</th>
  </tr>
</table>
<form action="loadTableDownload.xhtml?sysComit=${sysComit?string}" method="post" name="tradeTypeForm" id="tradeTypeForm">
...
<div class="menu">
	<div style="float:left;">
  	<ul>
	<li id="img_left"></li>
	<#if sysComit><li id="img_save">		<a href="#" class="b1" onClick="createTrade('createTableDownloadInit.xhtml?sysComit=${sysComit?string}');"><b>添加</b></a></li>
	<li id="img_amend">		<a href="#" class="b2" onClick="loadNote('createTableDownloadInit.xhtml?sysComit=${sysComit?string}&daActualTableNotice.id');"><b>修改</b></a></li>
	<li id="img_del">		<a href="#" class="b3" onClick="loadNote('deleteTableDownload.xhtml?sysComit=${sysComit?string}&daActualTableNotice.id');"><b>删除</b></a></li></#if>
...
<form action="deleteTableDownload.xhtml" method="post" name="tradeTypesForm" id="tradeTypesForm">
  	...
  	<#list actualizeTableNoticeList as actualizeTableNotice>
  		<tr>
	    	...
	    	<td><#if pagination.itemCount?exists>${pagination.itemCount+actualizeTableNotice_index+1}<#else>${actualizeTableNotice_index+1}</#if></td>
  			<td><div align="left"><a href="createTableDownloadInit.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&sysComit=${sysComit?string}">${actualizeTableNotice.title}&nbsp;</a></div></td>
  			<td>&nbsp;${actualizeTableNotice.createTimeBegin}</td>
 			<!--<#if sysComit><td><#if actualizeTableNotice.auditing><a href="updateTableDownload.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&state=false&sysComit=${sysComit?string}">取消审核</a><#else><a href="updateTableDownload.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&state=true&sysComit=${sysComit?string}">审核</a></#if></td></#if>-->
  		</tr>
  	</#list>
  </#if>
</table>
