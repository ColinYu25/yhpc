...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_4_blue">
  <tr>
	<th width="100%" height="22">法规标准列表</th>
  </tr>
</table>
<form action="loadLow.xhtml?sysComit=${sysComit?string}" method="post" name="tradeTypeForm" id="tradeTypeForm">
...
<div class="menu" style="width:100%;">
	<div style="float:left;">
  	<ul>
	<li id="img_left"></li>
	<#if sysComit><li id="img_save"><a href="#" class="b1" onClick="createTrade('createLowInit.xhtml?sysComit=${sysComit?string}');"><b>添加</b></a></li>
	<li id="img_amend">		<a href="#" class="b2" onClick="loadNote('createLowInit.xhtml?sysComit=${sysComit?string}&daActualTableNotice.id');"><b>修改</b></a></li>
	<li id="img_del">		<a href="#" class="b3" onClick="loadNote('deleteLow.xhtml?sysComit=${sysComit?string}&daActualTableNotice.id');"><b>删除</b></a></li></#if>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_list">
  <tr>
    ...
    <th width="15%" nowrap>序号</th>
    <th width="40%" nowrap>标题</th>
    <th width="20%" nowrap>发布时间</th>
  	...
  	<#list actualizeTableNoticeList as actualizeTableNotice>
  		<tr>
	    	...
	    	<td><#if pagination.itemCount?exists>${pagination.itemCount+actualizeTableNotice_index+1}<#else>${actualizeTableNotice_index+1}</#if></td>
  			<td><div align="left"><a href="createLowInit.xhtml?daActualTableNotice.id=${actualizeTableNotice.id}&sysComit=${sysComit?string}">${actualizeTableNotice.title}&nbsp;</a></div></td>
