...
<#escape x as (x)!> 
<@fkMacros.loading />
<#assign sysUrl="http://60.190.2.104/nbajj_colligation/companyInfo"> 
  ...
  <!--内容右侧 列表box开始-->
    <td valign="top">
      <div id="right_box">
        ...
        <form action="loadMapForSearch.xhtml" method="post" name="searchForm" id="searchForm">
	        <table width="100%" class="tab_form">
	          ...
	          <tr>
	            ...
	            <td colspan="2"><div id="div-area"></div></td>
	          </tr>
	          <tr>
        ...
        <table width="100%" class="tab_list_map" id="map_list">
          <#if companys?exists>
  			<#list companys?if_exists as company>
  			 <tr id="show_box" onClick="javascript:popZfMarker('${company.companyName}','${company.id}','company','${sysUrl}/loadCompanyInfoForZF.xhtml?company.id=${company.id}');">
...
// 	var pointArr = new Array();
	<#list mapMarkers?if_exists as marker>
		...
		var markerLink = "${sysUrl}/loadCompanyInfoForZF.xhtml?company.id=${marker.markerId}";
...
//	map.clearOverlays(); //清除地图上所有标记
	var markerLink = "${sysUrl}/loadCompanyInfoForZF.xhtml?company.id=" + markerId;
