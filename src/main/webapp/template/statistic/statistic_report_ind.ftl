	...
	function ShowTabss(obj,tabs,ID){ 
		...
		function showArea(areaCode) {
			if("${area.gradePath}".split("/").length-1 == 4) {
				...
				window.location = "loadReportByInd.xhtml?statistic.remark=${statistic.remark}&area.areaCode="+areaCode+"&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter=${statistic.quarter}";
			}
		}
		...
		function changeQUrl() {
			window.location = "loadReportByInd.xhtml?statistic.remark=${statistic.remark}&area.areaCode=${area.areaCode}&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter="+arguments[0];
		}
		
		function winLink() {
			window.location = "loadReportByIndAll.xhtml";
		}
