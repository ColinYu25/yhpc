	...
	<div class="box-con">
					<div class="box-title7">
							...
							<div class="fn2">
								<div class="left">
						    		...
						    		<b><#assign title=""/>
							            ...
							            ${title}${s.enumName}季报进度<#assign name='${s.enumName}'></b>
								</div>
								<div class="right">
									<ul class="fnlist">
						...
						<table width="986" border="0" cellspacing="0" cellpadding="0" class="table01">
						  <tr>
							<td class="tbbg"><b>${name}</b>（${title}）
							<#if refreshDate??><strong> <font  color=red > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 最近更新时间： </font>	</strong>${refreshDate?datetime("yyyy-MM-dd HH:mm:ss")}</#if>	</strong>		
							&nbsp;&nbsp;
							<#if refresh==1 >
								<a href="loadQuarterAndItemByIndustry.xhtml?<#if secondStatistic??>secondStatistic=${secondStatistic}&</#if><#if area.areaCode??>area.areaCode=${area.areaCode}&</#if><#if statistic.year??>statistic.year=${statistic.year}&</#if><#if statistic.quarter??>statistic.quarter=${statistic.quarter}&</#if><#if statistic.remark??>statistic.remark=${statistic.remark}&</#if><#if statistic.isSonType??>statistic.isSonType=${statistic.isSonType}&</#if>statistic.reFresh=true"><strong><font  color=red >刷新</font></strong></a>
							</#if>
							</td>
						  </tr>
						  <tr>
						...
						<table width="986" border="0" cellspacing="0" cellpadding="0" class="table01">
						  ...
						  <tr>
							...
							<#if area.areaCode==330200000000&&statistic.remark!='shuili'>
								...
								<tr class="table02" bgcolor="#F5F5F5" align="center">
									<#list areasByGroupNum?if_exists as a>
									<#if (a_index<5)>
										<#if (a.areaCode==0)>
								...
								<#else>
									...
									<#list 0..3 as i>
										  ...
										  <#elseif i_index==1><td width="6%" nowrap="true"><strong>未季报数</strong></td><#assign ptype='paicha'><#assign isGorver='0'>
	...
	<#if area.areaCode==330200000000>roleName();</#if>
		function showArea(areaCode) {
			if("${area.gradePath}".split("/").length-1 == 4) {
				...
				window.location = "loadQuarterAndItemByIndustry.xhtml?statistic.isSonType=1&statistic.remark=${statistic.remark}&area.areaCode="+areaCode+"&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter=${statistic.quarter}";
			}
		}
		...
		function changeQUrl() {
			window.location = "loadQuarterAndItemByIndustry.xhtml?statistic.isSonType=1&statistic.remark=${statistic.remark}&area.areaCode=${area.areaCode}&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter="+arguments[0];
		}
		
		function showMonthData(month) {
			window.location = "loadQuarterAndItemByIndustry.xhtml?statistic.isSonType=1&statistic.remark=${statistic.remark}&area.areaCode=${area.areaCode}&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter=${statistic.quarter}&statistic.month=" + month;
		}
	</script>
