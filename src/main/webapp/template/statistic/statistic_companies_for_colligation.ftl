...
<div class="list_title_blue" >		${area.areaName}生产经营单位统计  
	...
	<div id="main_content">
					...
					<form  style="margin-bottom:0px;margin-top:0px;" action="loadCompanysForColligation.xhtml?area.areaCode=${area.areaCode}" method="post" name="companyForm" id="companyForm"> 
					   	  ...
					   	  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_tab">
							  ...
							  <#if (companies?if_exists?size > 0)>
								...
								<tr class="table03" <#if co_index%2==0>bgcolor="#ffffff"<#else>bgcolor="#F5F5F5"</#if>>
									...
									<td><div align="left"><a href="#" id="isFrame${co.id}" onClick="OpenCompanyWindow('isFrame${co.id}','${co.id}');">${co.companyName}</a></td>
									<td><div align="left">${co.regAddress}</td>
									<td><#if co.fddelegate?exists>${co.fddelegate}</#if></td>
