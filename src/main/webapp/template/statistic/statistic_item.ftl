			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			 <#if area.areaCode==330200000000>
			  <tr class="table02"><td width="15%" height="28" colspan="2" rowspan="1">区域</td>
				...
				<#else>
				<tr class="table02"><td colspan="2"><strong>区域</strong></td>
			  	<#list areasByGroupNum?if_exists as a>
				...
				<#list 0..3 as i>
					  ...
					  <#elseif i_index==1><td width="6%" nowrap="true"><#if menuName?exists>${menuName}</#if>未录入数</td><#assign ptype='paicha'><#assign isGorver='0'>
					  <#elseif i_index==2><td width="6%" nowrap="true"><#if menuName?exists>${menuName}</#if>录入率</td>
	...
	<#--<#if area.areaCode==330200000000>roleName();</#if>-->
		function showArea(areaCode) {
			if("${area.gradePath}".split("/").length-1 == 4) {
				...
				window.location = "loadItemByIndustry.xhtml?statistic.remark=${statistic.remark}&area.areaCode="+areaCode+"&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter=${statistic.quarter}";
			}
		}
		...
		function changeQUrl() {
			window.location = "?statistic.isStType=${statistic.isStType}&statistic.remark=${statistic.remark}&area.areaCode=${area.areaCode}&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter="+arguments[0];
		}
		/**
		*根据年份查询统计表
		*/
		function showMonthData(month) {
			window.location = "?statistic.isStType=${statistic.isStType}&statistic.remark=${statistic.remark}&area.areaCode=${area.areaCode}&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter=${statistic.quarter}&statistic.month=" + month;
		}
	</script>
