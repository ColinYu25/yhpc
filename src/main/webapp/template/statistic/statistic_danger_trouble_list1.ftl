...
<body>&nbsp;
	<div class="main">
		<div class="box">
				...
				<div class="box-con">  
					<form  style="margin-bottom:0px;margin-top:0px;" action="loadDangerTroubleByTypeList.xhtml?statistic.isSonType=${statistic.isSonType}&statistic.year=${statistic.year}&statistic.beg_month=${statistic.beg_month}&statistic.end_month=${statistic.end_month}&area.areaCode=${area.areaCode}&statistic.isGorver=${statistic.isGorver}&statistic.isRollcall=${statistic.isRollcall}" method="post" name="companyForm" id="companyForm">
					  ...
					  <tr>
						<td colspan="4">
					   	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
							  ...
							  <#if dangers?exists && dangers?size gt 0>
								  <#list dangers?if_exists as c>
								   <tr class="table03" <#if c_index%2==0>bgcolor="#ffffff"<#else>bgcolor="#F5F5F5"</#if>>
								    ...
								    <td><div align="left"><#if c.daCompanyPass?exists >${c.daCompanyPass.daCompany.companyName}<#else>${c.companyName}</#if>&nbsp;</div></td>
								    <td nowrap title="点击查看重大事故隐患情况表" nowrap onClick="showDanger(${c.id});" style="cursor:pointer;">${c.dangerNo}&nbsp;</td>
								    <td><div align="left"><#if c.dangerAdd?exists>${c.dangerAdd}<#else>&nbsp;</#if></div></td>
								    <td nowrap><#if c.finishDate?exists>${c.finishDate.toString().substring(0,10)}</#if>&nbsp;</div></td>
								   <#--  
								    <td nowrap title="点击查看重大事故隐患治理情况表" nowrap onClick="showDangerGorver(${c.id});" style="cursor:pointer;"><#if c.daCompanyPass.daCompany.companyCode?exists>已<#elseif  c.isGorver?exists >已<#else>未</#if>&nbsp;</td>
								    <td nowrap><#if c.daDangerGorver?exists && c.daDangerGorver.finishDate?exists>${c.daDangerGorver.finishDate.toString().substring(0,10)}<#elseif c.daCompanyPass.daCompany.companyCode?exists >${c.daCompanyPass.daCompany.companyCode}<#else>-</#if>&nbsp;</td>
								    -->
								    
								    <td nowrap title="点击查看重大事故隐患治理情况表" nowrap onClick="showDangerGorver(${c.id});" style="cursor:pointer;"><#if   c.daDangerGorver?exists && c.daDangerGorver.finishDate?exists >已<#else>未</#if>&nbsp;</td>
								    <td nowrap><#if c.daDangerGorver?exists && c.daDangerGorver.finishDate?exists>${c.daDangerGorver.finishDate.toString().substring(0,10)}<#else>-</#if>&nbsp;</td>
								    <td nowrap>
								    <#if c.isOver?exists>
									    <#if c.isOver=='1' ||  c.daCompanyPass.daCompany.flag?exists  &&  c.daCompanyPass.daCompany.flag=='1'>
										    <#if c.daDangerGorver?exists && c.daDangerGorver?size gt 0&& c.daDangerGorver.finishDate?exists>
								    ...
								    <td nowrap title="点击查看挂牌督办程序列表" nowrap onClick="showRollcallCompanies(${c.id});" style="cursor:pointer;"><#if (c.daRollcallCompanies?exists &&c.daRollcallCompanies?size>0)>是<#elseif  c.daCompanyPass.daCompany.affrim?exists >是<#else>否</#if>&nbsp;</td>
								  </tr>
								  </#list>
							  <#else>
								<tr class="table03">
