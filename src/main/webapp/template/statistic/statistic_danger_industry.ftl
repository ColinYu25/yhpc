...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_list2">
    		...
    		<th width="5%" nowrap="true"><strong>合计</strong></th></tr>
			  ...
			  <tr height="25" <#if hy_index%2==0>bgcolor="#ffffff"<#else>bgcolor="#f0f0f0"</#if> align="center">
			  	<#if i_index==0>
			  		...
			  		<td rowspan="${hy.sonCount*4}" width="3%"><strong>
			  			<#list 0..hy.enumName?length-1 as hl>${hy.enumName?substring(hl_index,hl_index+1)}<br></#list>
			  		</strong></td>
			  		<td rowspan="4" colspan="${colParam}"><strong>合计</strong></td>
			  		<#else><#if ((hy.sonNumber1+hy.sonNumber2)==0 && hy_index=7)><#assign colParam=colParam+1></#if><td rowspan="4" <#if colParam?exists>colspan="${colParam}"<#else>colspan="1"</#if>><strong>${hy.enumName}</strong></td>
			  		</#if>
			  	...
			  	<#elseif i_index==2>
			  		<td nowrap="true" width="6%"><strong>到期未整改数</strong></td><#assign isGorver='2'>
		  		<#elseif i_index==3>
			  		<td nowrap="true" width="6%"><strong>整改率</strong></td>
			  	</#if>
			  	<#assign rowTotal=0>
			  	<#list areas?if_exists as a>
			    <td <#if i_index!=3>onClick="showDetail(${a.areaCode},'${hy.industryId}','${isGorver}');"  style="cursor:hand;"</#if>>
			    <#list statistics?if_exists as s>
			    ...
			    <#elseif i_index==2>${s.zero}<#assign rowTotal=rowTotal+s.zero>
			    <#elseif i_index==3><#if s.inhere==0>/<#else>#{(s.number/s.inhere*100);M1}%</#if></#if></#if></#if></#list></td>
			    </#list>
			    <td <#if i_index!=3>onClick="showDetail(${area.areaCode},'${hy.industryId}','${isGorver}');"  style="cursor:hand;"</#if>>
			    <#if i_index==0>${rowTotal}<#assign inhere=rowTotal>
			    <#elseif i_index==1>${rowTotal}<#assign number=rowTotal>
			    <#elseif i_index==2>${rowTotal}<#assign zero=rowTotal>
			    <#elseif i_index==3><#if inhere==0>/<#else>#{((inhere-number)/inhere*100);M1}%</#if></#if></td>
			  </tr>
			 </#list>
	...
	<script type="text/javascript">
		...
		function showDetail(areaCode,industryId,isGorver) {
			window.location = "loadDangerByIndustryList.xhtml?statistic.year=${statistic.year}&area.areaCode="+areaCode+"&statistic.industryId="+industryId+"&statistic.isGorver="+isGorver;
		}
		function changeQUrl() {
