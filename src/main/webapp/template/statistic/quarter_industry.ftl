					...
					<div class="box-title7">
							...
							<div class="fn2">
								<div class="left">
									...
									<b> <#assign title=""/>
							            ...
							            ${title}<#if area.fatherId==3020  && s.enumName=="贸易局">商务局<#elseif area.fatherId==3020  && s.enumName=="贸易">商务<#else>${s.enumName}</#if>季报进度</b>
								</div>
								<div class="right">
									<ul class="fnlist">
						...
						<table width="986" border="0" cellspacing="0" cellpadding="0" class="table01" >
						  <tr>
							...
							&nbsp;&nbsp;
							<#if refresh==1 >
								<a href="loadQuarterByIndustry.xhtml?<#if secondStatistic??>secondStatistic=${secondStatistic}&</#if><#if area.areaCode??>area.areaCode=${area.areaCode}&</#if><#if statistic.remark??>statistic.remark=${statistic.remark}&</#if><#if statistic.year??>statistic.year=${statistic.year}&</#if><#if statistic.quarter??>statistic.quarter=${statistic.quarter}&</#if><#if statistic.month??>statistic.month=${statistic.month}&</#if><#if statistic.isSonType??>statistic.isSonType=${statistic.isSonType}&</#if>statistic.reFresh=true"><strong><font  color=red >刷新</font></strong></a>
							</#if>	
							</td>
						  </tr>
						  <tr>
							<td>
							<table width="100%" border="0" cellspacing="0" cellpadding="0" style="overflow-y:scroll;overflow-y:scroll;">
							  <tr class="table02">
							  	<td width="15%" height="28" <#if statistic.remark=='anjian'>colspan="${s.sonNumber1+s.sonNumber2+1}"<#else>colspan="${s.sonNumber1+s.sonNumber2+2}"</#if>><#if s_index==0><#break></#if></#list>区域</td>
								  ...
								  <tr class="table03" <#if hy_index%2==0>bgcolor="#ffffff"<#else>bgcolor="#F5F5F5"</#if> align="center">
								  	<#if i_index==0>
								  		...
								  		<#if statistic.remark == 'anjian'>
								  			<td rowspan="3" width="3%" colspan="2"><strong>
								  		<#else>
								  			<td rowspan="${hy.sonCount*3}" width="3%"><strong>
								  		</#if>
								  			<#list 0..hy.enumName?length-1 as hl><#if area.fatherId==3020  && hy.enumName?substring(hl_index,hl_index+1)=='贸'>商<#elseif area.fatherId==3020  && 

hy.enumName?substring(hl_index,hl_index+1)=='易'>务<#else>${hy.enumName?substring(hl_index,hl_index+1)}</#if><br></#list>
								  		</strong></td>
								  		<#if statistic.remark == 'anjian'>
								  		<#else>
      ...
      if(o.propertyName!='value'){
		...
		function showArea(areaCode) {
			if("${area.gradePath}".split("/").length-1 == 4) {
				...
				window.location = "loadQuarterByIndustry.xhtml?statistic.isSonType=1&statistic.remark=${statistic.remark}&area.areaCode="+areaCode+"&statistic.year="+get("year").value.substring(0,4)+"&statistic.quarter=${statistic.quarter}";
			}
		}
