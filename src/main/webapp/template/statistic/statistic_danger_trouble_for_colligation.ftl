	...
	<div id="main_content">
			    ...
			    <table class="table02" style="background:#F5F5F5;">
				    <tr>
				    	<td nowrap width="20%" style="font-size:16px;border-right:0px solid #AED4F3;font-weight:bold;border-bottom:0px solid #AED4F3;"><div style="text-align:center">&nbsp;</div></td> 
				      	...
				      	<td nowrap width="20%" style="font-size:16px;border-right:0px solid #AED4F3;font-weight:bold;border-bottom:0px solid #AED4F3;"><div style="text-align:center">${statistic.year}年<#if statistic.beg_month!=0>${statistic.beg_month}月</#if>到<#if statistic.end_month!=0>${statistic.end_month}月</#if><#if statistic.beg_month==0 && statistic.end_month==0>${statistic.year + 1}年</#if></div></td> 
				     </tr>
			    </table>
			    </form>-->
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_tab">
				  <tr class="tbbg" style="font-size:16px;font-weight:bold;">
		       		...
		       		<a style="margin-left:10px;margin-right:10px;float:right;" class="b6" href="loadNomalTroubleForColligation.xhtml?area.areaCode=${area.areaCode}&statistic.isSonType=${statistic.isSonType}&statistic.year=${statistic.year}&statistic.quarter=${statistic.quarter}&statistic.isRollcall=${statistic.isRollcall}&statistic.isGorver=${statistic.isGorver}">
		       		<b><img style="border:0px" src="${resourcePath}/images/021.gif" /></b></a>
		       	</td>
				  </tr>
				  <tr>
					<td colspan="4">
				   	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
						  	<tr class="table02">
							    ...
							    <td width="25%" nowrap>企业名称</td>
							    <td width="10%" nowrap>隐患编号</td>
							    <td width="25%" nowrap>隐患地址</td>
							    <td width="10%" nowrap>计划治理期限</td>
							    <td width="5%" nowrap>治理状态</td>
							    <td width="8%" nowrap>实际治理日期</td>
							    <td width="7%" nowrap>是否超期</td>
							    <td width="5%" nowrap>是否挂牌督办</td>
							</tr>
						  <#if dangers?exists && dangers?size gt 0>
							  <#list dangers?if_exists as c>
							   <tr class="table03" <#if c_index%2==0>bgcolor="#ffffff"<#else>bgcolor="#F5F5F5"</#if>>
							    ...
							    <#if c.isOver?exists>
								    <#if c.isOver=='1'>
									    <#if c.daDangerGorver?exists && c.daDangerGorver?size gt 0>
				...
				<div style="margin-left:-5px;text-align:left">
					<table  width="986"  cellpadding="0" cellspacing="5">
						<tr>
						<td >
							<table width="984" border="0" cellpadding="0" cellspacing="0" style="border: #CCCCCC solid 1px; border-collapse: collapse; background: #f6f6f6; font-size: 12px;line-height:150%;">
								<tr>
