...
<form  action="excel1.xhtml?area.areaCode=${area.areaCode}<#if company.industryId?exists && company.industryId!=0>&company.industryId=${company.industryId}</#if>&company.companyTrade=${company.companyTrade}" method="post" name="exportForm" id="exportForm">
		...
		<input type="hidden" id="year" name="company.year"  value="${company.year}"/>
		<input type="hidden" id="quarter" name="company.quarter"  value="${company.quarter}"/>
...
<form  action="?&area.areaCode=${area.areaCode}<#if company.industryId?exists && company.industryId!=0>&company.industryId=${company.industryId}</#if>&company.companyTrade=${company.companyTrade}" method="post" name="companyForm" id="companyForm">
	<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_input">
	  ...
	  <tr>
	    <th>年  份</th>
	    <td width="35%">
	    <input type="text" id="company.year" name="company.year" onChange="winLocation();" value="${company.year}" onfocus="WdatePicker({dateFmt:'yyyy',minDate:'2010-01',maxDate:'%y-%M-%d %H-%m'})" class="Wdate" size="9" maxlength="8">
	             &nbsp; &nbsp;&nbsp;&nbsp;季  度
	    <select  id="company.quarter" name="company.quarter" >
	       <option value="1" <#if  company.quarter==1  > selected </#if> >1</optipon>
	       <option value="2" <#if  company.quarter==2  > selected </#if> >2</optipon>
	       <option value="3"  <#if  company.quarter==3  > selected </#if>>3</optipon>
	       <option value="4"  <#if  company.quarter==4  > selected </#if>>4</optipon>
	    </select>
	    </td>
	    <th>区     域</th>
...
<div class="menu">
	...
	<li id="img_refurbish"><a href="#" class="b4" onClick="qexport();"><b>导出</b></a></li>
