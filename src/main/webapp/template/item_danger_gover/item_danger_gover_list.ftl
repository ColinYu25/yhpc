...
<table border="0" cellpadding="0" cellspacing="0" class="table_4_blue">
  <tr>
	...
	<#elseif flag='chengguan'>
		市政
	</#if>工程施工重大事故隐患已治理列表</th>
  </tr>
</table>
<form action="loadGoverItems.xhtml" method="post" name="itemsForm" id="itemsForm">
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_list">
  <tr>
    ...
    <th width="13%" style="cursor:hand;" nowrap>总承包单位</th>
    <th width="8%" style="cursor:hand;" nowrap>重大隐患数</th>
    <th width="8%" style="cursor:hand;" nowrap>已治理重大隐患数</th>
    <th width="8%" style="cursor:hand;" nowrap>未治理重大隐患数</th>
  </tr>
  <#if items?exists>
	  ...
	  <tr>
	    ...
	    <td><a id="tp5${c.id}" href="../itemDanger/loadItemDangers.xhtml?itemDanger.govern=2&item.id=${c.id}&itemDanger.daItem.id=${c.id}"></a></td>
	  	<td><a href="../itemDanger/loadItemDangers.xhtml?itemDanger.govern=1&item.id=${c.id}&itemDanger.daItem.id=${c.id}">${c.goverDangernNum}</a></td>
	 	<td><a href="../itemDanger/loadItemDangers.xhtml?itemDanger.govern=0&item.id=${c.id}&itemDanger.daItem.id=${c.id}">${c.daItemDangers.size()-c.goverDangernNum}</a></td>
	  </tr>
	 </#list>
  </#if>
</table>
