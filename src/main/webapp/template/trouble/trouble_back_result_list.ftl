...
<#escape x as (x)!>
<#if dept?exists>
	<#if  dept.type?if_exists=='1'>
		<#assign letter='隐患回复列表'>
		<#assign url='loadTroubleBack.xhtml'>
	<#elseif  dept.type?if_exists=='3'>
		<#assign letter='隐患反馈列表'>
		<#assign url='loadTroubleResult.xhtml'>
	</#if>
</#if>
...
<div class="menu">
  	...
  	<#if dept.type?if_exists=='1'>
  	<li id="img_vary"><a href="#" class="b8" onClick="loadNote('createTroubleInitByBack.xhtml?dept.id')"><b>回复</b></a></li>
  	</#if>
  	<#if dept.type?if_exists=='3'>
  	<li id="img_report"><a href="#" class="b7" onClick="loadNote('createTroubleInitByResult.xhtml?dept.id')"><b>反馈</b></a></li>
  	</#if>
