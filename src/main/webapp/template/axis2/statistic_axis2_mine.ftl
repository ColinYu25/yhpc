  ...
  <tr id="sendIpt" style="display:none" >
   ...
   <tr>
	...
	<#if refreshDate??><strong> <font  color=red > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 最近更新时间： </font>	</strong>${refreshDate?datetime("yyyy-MM-dd HH:mm:ss")}</#if></strong>	
		&nbsp;&nbsp;
	<#if refresh==1 >
		<a href="loadStatisticForAxis2MineByParam.xhtml?statistic.yearMonth=${statistic.yearMonth}&statistic.reFresh=true"><strong><font  color=red >刷新</font></strong></a>
	</#if>
	</td>
  </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0" class="table_list">
		...
		<#assign sccchgovhgqy_governMoney = (map["s_7"].governMoney + map["s_9"].governMoney+map["s_7"].normalGovernMoney + map["s_9"].normalGovernMoney)>
		 ...
		 <#assign num = (map["s_3"].companyNum + map["s_4"].companyNum)>
		  ...
		  <td align="center" id="bt"><#if (map["s_3"].troubCleanNum + map["s_4"].troubCleanNum)==0>/<#else>#{((map["s_3"].troubCleanNum + map["s_4"].troubCleanNum)/(map["s_3"].troubCleanNum + map["s_4"].troubCleanNum)*100);M1}</#if></td>
		  ...
		  <td align="center" id="pgp">${map["s_3"].proviceRcNum + map["s_3"].cityRcNum + map["s_3"].qtRcNum  + map["s_4"].proviceRcNum + map["s_4"].cityRcNum+ map["s_4"].qtRcNum}</td>
...
<script>
	...
	function checkOldData(){
		window.location="loadStatisticForAxis2MineByParam2.xhtml?statistic.yearMonth="+get("year").value;
		progressText("正在加载统计数据，请稍等...");
	}
	...
	var url = "saveSendTime.xhtml?statistic.setTime="+document.getElementById("sendDateTime").value+"&statistic.type=1&statistic.yearMonth="+get("year").value;
			jQuery.ajax({
			   type: "POST",
			   url: url,
			   dataType:"text",
