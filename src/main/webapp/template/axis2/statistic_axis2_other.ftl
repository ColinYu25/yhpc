...
<table width="30%" border="0" height="30" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<td align="center">
		<input name="print" id="sendData" type="button" <#if send?exists>disabled="disabled"<#else></#if> value="<#if send?exists&&send.isSend==1>已经发送<#else>设置发送时间</#if>" onclick="sendData();"/>
	</td>
	</#if>
	<td align="center">
   ...
   <tr id="sendIpt" style="display:none" >
  	<td  align="center"  colspan=4>
   		<input name="sendDateTime" id="sendDateTime"  type="text"  size=10 onfocus="WdatePicker({minDate:'#F{\'%y-{%M}-%d\'}',dateFmt:'yyyy-MM-dd',maxDate:'%y-{%M}-%ld',errDealMode:1});" />&nbsp;&nbsp;&nbsp;<input name="save1" id="save1"  type="button" value="确 定"  onclick="showSend();" />
   </td>
  </tr>
   <tr id="sendContent"  <#if send?exists> <#if send.isSend==1> style="display:none"</#if>  <#else>style="display:none"</#if> >
  	<td  align="center"  colspan=4  > 
   		<font color="red"><span id=content1>该报表将于<#if send?exists>${send.setTime}</#if>晚22点上报至省隐患排查系统,请注意查看!</font></font>
   </td>
  </tr>
  <tr>
  	<td align="center" colspan=4>
	<#if refreshDate??><strong> <font  color=red > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 最近更新时间： </font>	</strong>${refreshDate?datetime("yyyy-MM-dd HH:mm:ss")}</#if></strong>	
		&nbsp;&nbsp;
	<#if refresh==1 >
		<a href="loadStatisticForAxis2OtherByParam.xhtml?statistic.yearMonth=${statistic.yearMonth}&statistic.reFresh=true"><strong><font  color=red >刷新</font></strong></a>
	</#if>
	</td>
  </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0" class="table_list">
        ...
        <#assign all_num = (map["s_total"].allCompanyNum)>
		 ...
		 <#assign num = (map["s_19"].companyNum)>
		  ...
		  <td align="center" id="pgp">${map["s_19"].proviceRcNum + map["s_19"].cityRcNum + map["s_19"].qtRcNum + map["s_19"].qtRcNum }</td>
...
<script language="javascript">
	...
	function sendData(){
		...
		document.getElementById("sendIpt").style.display="block"; 
	...
	function showSend(){
	var sendDateTime=document.getElementById("sendDateTime").value;
	if(sendDateTime==''){
	   alert("请选择发送时间！");
	   return false;
	}
	var url = "saveSendTime.xhtml?statistic.setTime="+document.getElementById("sendDateTime").value+"&statistic.type=2&statistic.yearMonth="+get("year").value;
			jQuery.ajax({
			   type: "POST",
			   url: url,
			   dataType:"text",
		...
		document.getElementById("sendData").disabled=true; 
		document.getElementById("sendIpt").style.display="none"; 
		
		document.getElementById("content1").innerHTML="该报表将于"+document.getElementById("sendDateTime").value+"晚22点上报至省隐患排查系统,请注意查看!";
