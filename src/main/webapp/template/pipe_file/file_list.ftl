...
<table width="98%" border="0" cellpadding="0" cellspacing="0" class="table_4_blue">
  <tr>
	<th width="98%" height="22"><#if type==1>督办方案<#elseif type==2>治理方案<#elseif type==3>督办记录</#if>列表</th>
  </tr>
</table>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_list">
  ...
  <#if rollcallFiles?exists>
  	<#list rollcallFiles as rollcallFile>
  		<#if rollcallFile.daPipeFile?exists>
  		<tr>
  			<td><input id="ids" name="ids" type="checkbox" value="${rollcallFile.daPipeFile.id}"/></td>
	    	<td><#if pagination.itemCount?exists>${pagination.itemCount+rollcallFile_index+1}<#else>${rollcallFile_index+1}</#if></td>
  			<td align="center"><a href="${contextPath}${rollcallFile.daPipeFile.fileRealPath}">${rollcallFile.daPipeFile.fileName}</a></td>
  			<td>${rollcallFile.daPipeFile.modifyTime}&nbsp;</td>
  		</tr>
  		</#if>
  	</#list>
  </#if>
