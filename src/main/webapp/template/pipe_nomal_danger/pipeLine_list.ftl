...
<div class="menu">
  	<ul>
	<li id="img_xcjcjl"><a class="b_xcjcjl" href="javaScript:loadNote('createNomalDangerInit.xhtml?entity.id')"><b>添加隐患</b></a></li>
	<#if company?? && company.id??>
    <li id="img_zgtzs"><a class="b_zgtzs" href="javaScript:addWuNomalDangers(get('pipeNomalDangerForm'), 'addWuNomalDangers.xhtml')"><b>添加无隐患</b></a></li>
  	</#if>
	<li id="img_xcjcjl"><a class="b_xcjcjl" href="javaScript:loadNote('loadNomalDangers.xhtml?entity.id')"><b>隐患列表</b></a></li>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list">
  <tr>
    ...
    <th width="15%" nowrap>管道名称</th>
    <th width="5%" nowrap>隐患数</th>
  ...
  <#if result?exists>
	  ...
	  <tr>
	    ...
	    <td style="text-align:center">
	    	<#if item.type?? && item.type gt 0>
	    		<a href="../pipeline/yq_pipeline_update.xhtml?entity.id=${item.id}">${item.name}</a>
	    	<#else>
	    		<a href="../pipeline/rq_pipeline_update.xhtml?entity.id=${item.id}">${item.name}</a>
	    	</#if>
	    	&nbsp;
	    </td>
	    <td title="查看管道隐患信息" onClick="checkDangerByPipeId(${item.id});" style="cursor:pointer;" onMouseOver="this.style.backgroundColor=trMoveColor;" onMouseOut="this.style.backgroundColor='';">${item.count}&nbsp;</td>
	    <td>
	    ...
	    <td>
	    	...
	    	中低压燃气管道
	    	<#elseif item?? && item.type?? && item.type == 1>
	    	长输管道
	    	<#elseif item?? && item.type?? && item.type == 2>
	    	城镇燃气管道
	    	<#elseif item?? && item.type?? && item.type == 3>
	    	工业管道
	    	<#elseif item?? && item.type?? && item.type == 4>
	    	港区内油气管道
	    	<#else>
	    	&nbsp;
	    	</#if>
	    ...
	    <td>
		    <#if item?? && item.buildType?? && item.buildType == 1>
		   	 	架空
		    <#elseif item?? && item.buildType?? && item.buildType == 2>
		   	 	埋地
		    <#elseif item?? && item.buildType?? && item.buildType == 3>
		    	地面
		    <#else>
		    	&nbsp;
		    </#if>
	    ...
	    <td>${item.endPoint}&nbsp;</td>
	    <td>${item.buildEndDate?string('yyyy-MM-dd')}<#if !item.buildEndDate?exists>&nbsp;</#if></td>	    	    	    
	  </tr>
	 </#list>
  </#if>
</table>
...
<script>
	function addWuNomalDangers() {
		if(chooseCheckBox()) {
			if(window.confirm('确定给选择的管道批量添加无隐患吗？')) {
				if(arguments[1])
	...
	function checkDangerByPipeId(id){
		if(id && id!="")
			window.location = "loadNomalDangers.xhtml?entity.id=" + id;
	}
</script>