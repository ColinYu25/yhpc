		...
		if ((config.AutoDetectPasteFromWord=="1") && BrowserInfo.IsIE55OrMore) {
			...
			if ( re.test(sHTML)){
				if ( confirm( lang["MsgPasteWordConfirm"] ) ){
					...
					var arr = showModalDialog("dialog/localupload.htm?action=paste", window, "dialogWidth:0px;dialogHeight:0px;help:no;scroll:no;status:no");
					if (arr){
						return false;
					}
				}
	...
	if (NewMode!=sCurrMode){
		...
		case "VIEW":
			if (NewMode=="TEXT"){
				...
				//alert("代码状态\n"+sBody);
				//alert(sBody); //切换到代码视图时，触发。
				sBody=setPic2Tag(sBody);
			}
			break;
		default:
...
function setPic2Tag(content){
sPos=content.indexOf("{新闻盒子:");
ePos=content.indexOf("}",sPos);
while(sPos>=0 && ePos>sPos)
{
	var subStr=content.substring(sPos,ePos+1);
	subStr=subStr.replace("{新闻盒子:","<box:Hello");
	subStr=subStr.replace(/"\}/g,"\"/>");
	//alert(subStr+"1");
	var prefix=content.substring(0,sPos);
	var endfix=content.substring(ePos+1,content.length);
	//alert(endfix);
	content=prefix+subStr+endfix;
	sPos=content.indexOf("{新闻盒子:");
	ePos=content.indexOf("}",sPos);
}
return content;
...
function setTag2Pic(content){
sPos=content.indexOf("<box:Hello");
ePos=content.indexOf("/>",sPos);
while(sPos>=0 && ePos>sPos)
{
	var subStr=content.substring(sPos,ePos+2);
	subStr=subStr.replace("<box:Hello","{新闻盒子:");
	subStr=subStr.replace(/"\/>/g,"\"}");
	//alert(subStr);
	var prefix=content.substring(0,sPos);
	var endfix=content.substring(ePos+2,content.length);
	//alert(endfix);
	content=prefix+subStr+endfix;
	sPos=content.indexOf("<box:Hello");
	ePos=content.indexOf("/>",sPos);
