...
if(sAction.equals("UPDATECONFIG")){
	...
	String s_FormStateFlag, s_FormDetectFromWord, s_FormInitMode, s_FormBaseUrl, s_FormUploadObject, s_FormAutoDir, s_FormAutoRemote, s_FormShowBorder, s_FormAutoDetectLanguage, s_FormDefaultLanguage, s_FormSLTFlag, s_FormSYWZFlag, s_FormSLTSYObject, s_FormAllowBrowse, s_FormSYTPFlag, s_FormSYWZPosition, s_FormSYTPPosition, s_FormCusDirFlag;
	...
	s_FormAutoDir = GetSelect("d_autodir", split("不使用|年目录|年月目录|年月日目录", "|"), split("0|1|2|3", "|"), sStyleAutoDir, "");
	...
	s_FormSYWZFlag = GetSelect("d_sywzflag", split("使用|不使用", "|"), split("1|0", "|"), sSYWZFlag, "");
	s_FormSLTSYObject = GetSelect("d_sltsyobject", split("JDK1.4 ImageIO", "|"), split("0", "|"), sSLTSYObject, "");

	s_FormSYTPFlag = GetSelect("d_sytpflag", split("使用|不使用", "|"), split("1|0", "|"), sSYTPFlag, "");
	s_FormSYWZPosition = GetSelect("d_sywzposition", split("左上|左中|左下|中上|中中|中下|右上|右中|右下", "|"), split("1|2|3|4|5|6|7|8|9", "|"), sSYWZPosition, "");
	s_FormSYTPPosition = GetSelect("d_sytpposition", split("左上|左中|左下|中上|中中|中下|右上|右中|右下", "|"), split("1|2|3|4|5|6|7|8|9", "|"), sSYTPPosition, "");
	s_FormCusDirFlag = GetSelect("d_cusdirflag", split("禁用|启用", "|"), split("0|1", "|"), sCusDirFlag, "");
	...
	out.print("<table border=0 cellpadding=0 cellspacing=1 align=center class=form>" +
			...
			"<tr><td colspan=4><span class=red>&nbsp;&nbsp;&nbsp;上传相关设置（只有在使用相对路径模式时，才要设置显示路径和内容路径）：</span></td></tr>" +
			...
			"<tr><td width='15%'>本地类型：</td><td width='35%'><input type=text class=input name=d_localext size=20 title='用于自动上传本地文件' value='" + sStyleLocalExt + "'></td><td width='15%'>本地限制：</td><td width='35%'><input type=text class=input name=d_localsize size=20 title='数字型，单位KB' value='" + sStyleLocalSize + "'>KB</td></tr>" +
			...
			"<tr><td width='15%'>图片水印图片路径：</td><td width='35%'><input type=text name=d_sypicpath size=20 class=input title='当使用图片水印时图片的路径' value=\"" + sSYPicPath + "\"></td><td width='15%'>图片水印透明度：</td><td width='35%'><input type=text name=d_sytpopacity size=20 class=input title='0至1间的数字，如0.5表示半透明' value=\"" + sSYTPOpacity + "\"></td></tr>" +
			"<tr><td width='15%'>图片水印图片占位：</td><td width='35%'>宽:<input type=text name=d_sytpimagewidth size=4 class=input title='水印图片的宽度，数字型' value='" + sSYTPImageWidth + "'>px&nbsp; 高:<input type=text name=d_sytpimageheight size=4 class=input title='水印图片的高度，数字型' value='" + sSYTPImageHeight + "'>px&nbsp; <input type=button value='检测宽高' onclick='doCheckWH(2)'></td><td width='15%'></td><td width='35%'></td></tr>" +
	...
	if (!isNumber(sStyleLocalSize)) {
		out.print(getError("请填写有效的本地文件限制大小！"));
		return;
	}
	...
	if (!isNumber(sSYWZMinWidth)){
		out.print(getError("请填写有效的文字水印启用的最小宽度条件，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYWZMinHeight)){
		out.print(getError("请填写有效的文字水印启用的最小高度条件，不能为空，且为数字型！"));
	}
	if (sSYText.equals("")){
	...
	if (!isNumber(sSYWZPaddingH)){
		out.print(getError("请填写有效的文字水印左右边距，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYWZPaddingV)){
		out.print(getError("请填写有效的文字水印上下边距，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYWZTextWidth)){
		out.print(getError("请填写有效的文字水印文字宽占位，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYWZTextHeight)){
		out.print(getError("请填写有效的文字水印文字高占位，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYTPMinWidth)){
		out.print(getError("请填写有效的图片水印启用的最小宽度条件，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYTPMinHeight)){
		out.print(getError("请填写有效的图片水印启用的最小高度条件，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYTPPaddingH)){
		out.print(getError("请填写有效的图片水印左右边距，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYTPPaddingV)){
		out.print(getError("请填写有效的图片水印上下边距，不能为空，且为数字型！"));
	}
	if (!isFloat(sSYTPOpacity)){
		out.print(getError("请填写有效的图片水印透明度，不能为空，且为0至1的数字！"));
	}
	if (!isNumber(sSYTPImageWidth)){
		out.print(getError("请填写有效的图片水印文字宽占位，不能为空，且为数字型！"));
	}
	if (!isNumber(sSYTPImageHeight)){
		out.print(getError("请填写有效的图片水印文字高占位，不能为空，且为数字型！"));
	}
