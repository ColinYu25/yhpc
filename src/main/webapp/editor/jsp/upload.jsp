...
<%@page import="cn.safetys.sync.attach.AttachServer"%>
...
static String getUploadForm(String s_Type, String s_StyleName, String s_AllowExt, String s_Language, String s_CusDir){
	String html = "";
	html += "\n" + "<HTML>";
	html += "\n" + "<HEAD>";
	...
	html += "\n" + "<form action='?action=save&type=" + s_Type + "&style=" + s_StyleName + "&language=" + s_Language + "&cusdir=" + s_CusDir + "' method=post name=myform enctype='multipart/form-data'>";
...
if (sAction.equals("REMOTE")){
	...
	// show form
	out.print(getUploadForm(sType, sStyleName, sAllowExt, sLanguage, sCusDir));
	...
	try {
		...
		if (!CheckValidExt(sAllowExt, sFileExt)){
			out.print(getOutScript("parent.UploadError('lang[\"ErrUploadInvalidExt\"]+\":" + sAllowExt + "\"')"));
			out.close();
		}
...
//附件服务器上传文件
if(sPathFileName!=null&&!"".equals(sPathFileName.trim())
		&&SystemConstant.P!=null&&SystemConstant.P.isAttachUpload()){
	String __path = sPathFileName.replace(request.getContextPath()+"/", "");
	String realPath= request.getSession().getServletContext().getRealPath("/"+__path);//绝对路径
	//System.out.println("sPathFileName:"+sPathFileName);
	//System.out.println("__path:"+__path);
	//System.out.println("realPath:"+realPath);
	if(AttachServer.P==null){
		ServletContext context = request.getSession().getServletContext();
		ApplicationContext ctx = WebApplicationContextUtils.getWebApplicationContext(context);
		AttachServer attachServer = (AttachServer)ctx.getBean("attachServer");
		AttachServer.P = attachServer;
	}
	if(AttachServer.P!=null){
		AttachServer.P.asyncUpload(new File(realPath), __path);
	}
}
