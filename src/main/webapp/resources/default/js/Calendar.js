...
function writeIframe()
{
    ...
    "    <tr><td>&nbsp;</td><td align=center><input name=today type=button value='Today' onfocus='this.blur()' style='width: 50' title='当前日期&#13;快捷键：T'"+
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.objExport2  = null;                     //日历回传的显示控件2 //lv
    this.objExport3  = null;                     //日历回传的显示控件2 //lv
    this.objExport4  = null;                     //日历回传的显示控件2 //lv
...
function calendar(arg1,arg2,arg3,arg4) //
{
    var e = window.event.srcElement;   writeIframe();
    var o = WebCalendar.calendar.style; WebCalendar.eventSrc = e;
 if (arguments.length == 0) WebCalendar.objExport = e;
    else if(arguments.length == 1) WebCalendar.objExport = eval(arguments[0]);
    else if(arguments.length==2){  WebCalendar.objExport = eval(arguments[0]);WebCalendar.objExport2 = eval(arguments[1]);}
	else if(arguments.length==3){  WebCalendar.objExport = eval(arguments[0]);WebCalendar.objExport2 = eval(arguments[1]);WebCalendar.objExport3 = eval(arguments[2]);}
	else if(arguments.length==4){  WebCalendar.objExport = eval(arguments[0]);WebCalendar.objExport2 = eval(arguments[1]);WebCalendar.objExport3 = eval(arguments[2]);WebCalendar.objExport4= eval(arguments[3]);}
 ...
 while (e = e.offsetParent){t += e.offsetTop; l += e.offsetLeft;}
    ...
    if (dw + dl - l >= cw) {o.left = l;o.left = (o.left.substring(0,o.left.length-2) - 20)+"px";} else {o.left = (dw >= cw) ? dw - cw + dl : dl;o.left = (o.left.substring(0,o.left.length-2) - 20)+"px";}
...
function returnDate() //
{
    if(WebCalendar.objExport)
    {
		...
		/*if(a[2]==nowYear&a[1]>nowMonth){
			returnValue="";
			alert("不能大于当前日期");
		}else if(a[2]==nowYear&a[1]==nowMonth&a[0]>nowDate){
			returnValue="";
			alert("不能大于当前日期");
		}*/
        WebCalendar.objExport.value = returnValue;
		if(WebCalendar.objExport4!=null){
autoAddDate(WebCalendar.objExport,WebCalendar.objExport3,WebCalendar.objExport2,WebCalendar.objExport4);
		}
		if(WebCalendar.objExport3!=null){
			WebCalendar.objExport3.value =WebCalendar.objExport.value;
		}
        hiddenCalendar();
    }
}
...
function autoAddDate(date1,date2,date3,num){
	var Months= new Array(31,28,31,30,31,30,31,31,30,31,30,31); 
    var y=date1.value.substr(0,4);
    var m=date1.value.substr(5,2);
    var d=date1.value.substr(8,2);
	if (date1.vlue!=""){
    if (m==0 || d==0){
     alert("日期格式错误！");
	 date1.value="";
