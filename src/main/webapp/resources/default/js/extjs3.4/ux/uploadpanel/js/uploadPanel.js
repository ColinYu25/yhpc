...
//					    alert(grid);	
					    var grid =  Ext.getCmp('f_grid');
					    //console.log(grid.getTopToolbar().items.get(0));
		...
		UploadPanel.singleFile  = this.singleFile;
		UploadPanel.grid = this.fileList;
	},
...
//		});
	...
	onFileQueued : function(file) {
		...
		//单文件上传限制
		var  store = thiz.fileList.getStore();
		var tcount = store.getCount();
		if(UploadPanel.singleFile && tcount >= 1) {
			var record = store.getAt(0);
			//存在上传出错、或已经取消上传的文件直接清除store可以再次上传
			if( record.data.fileState == SWFUpload.FILE_STATUS.CANCELLED
			      || record.data.fileState == SWFUpload.FILE_STATUS.ERROR){
			     store.removeAll();
			}  else {
				 Ext.Mytip.msg('提示','只能上传一个文件！');
			     return;
			}
		}
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (res.success) {
			...
			record.set('fileDbId', res.fileDbId || ''); //附件表标识
			record.set('fileServerPath', res.fileServerPath || '');//上传到服务器路径
			record.commit();
		}
...
UploadPanel.singleFile =  false;
UploadPanel.grid = null;
