...
import com.test.ExportExcel;

@SuppressWarnings("serial")
public class ExcelNotQuarterReportAction extends ActionSupport {
	...
	public InputStream getExcelFile() {
		...
		try {
			...
			if (company != null) {
				...
				if (company.getYear() != 0l) {
					...
					if (company.getThirdArea() != null && company.getThirdArea() != 0) {
			...
			if (userDetail.getUserIndustry().equals("anwei") && ((company.getTradeTypes() != null && !"".equals(company.getTradeTypes()) && company.getTradeTypes().split(",")[0].trim().equals("")) || company.getTradeTypes() == null)) {
				...
				statistics = statisticFacadeIface.loadCompanyByIndustryList1(company, pagination, area, current_quarter, userDetail);
			}
		...
		try {
			this.fileName = new String("企业季报未上报情况汇总表".getBytes("gb2312"), "iso8859-1")+".xls";
